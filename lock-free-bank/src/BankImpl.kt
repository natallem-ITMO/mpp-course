...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    override fun withdraw(index: Int, amount: Long): Long {
        ...
        while (true) {
            ...
            check(account.amount - amount <= MAX_AMOUNT) { "Underflow" }
            val updated = Account(account.amount - amount)
            if (accounts[index].compareAndSet(account, updated)) return updated.amount
        }
    }
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
            ...
            val indexes: IntArray = intArrayOf(fromIndex, toIndex)
            val dindexes: IntArray
            if (fromIndex <= toIndex) {
                dindexes = intArrayOf(0, 1)
            } else {
                dindexes = intArrayOf(1, 0)
            }

            val accounts = arrayOf<AcquiredAccount?>(null, null)
            val numberOfAccounts = 2
            var acquired = 0
            while (acquired < numberOfAccounts) {
                val account = acquire(indexes[dindexes[acquired]], this) ?: break
                accounts[dindexes[acquired]] = account
                acquired++
            }
            if (acquired == numberOfAccounts) {
            ...
            for (i in 0 until numberOfAccounts) {
                release(indexes[dindexes[i]], this)
            }
